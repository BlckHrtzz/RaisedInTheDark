...
namespace EnhancedHierarchy {
    ...
    internal static class Styles {
        ...
        public static GUIStyle warnStyle = "CN EntryWarn";
        ...
        private const string treeLineBitmap = @"iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAYAAAAf8/9hAAAAJUlEQVQ4EWNgIAL8BwJcyphwSRArPmoAA8NoGIyGASi/DHw6AADYOwQbvk/7+AAAAABJRU5ErkJggg==";
        private const string treeEndBitmap = @"iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAYAAAAf8/9hAAAAK0lEQVQ4EWNgIAL8BwJcyphwSRArPmoAA8MwCANGYuIbX0IiRv+oGlqHAABHsQgKWP01jwAAAABJRU5ErkJggg==";
        private const string treeMiddleBitmap = @"iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAYAAAAf8/9hAAAAMUlEQVQ4EWNgIAL8BwJcyphwSRArPmoAA8PAhwEjMdGFLx0Qo58BnwEDHwajLmBgAADNbwwQNsi4sgAAAABJRU5ErkJggg==";
        ...
        public static Texture2D treeLine {
            get {
                if(!_treeLine)
                    _treeLine = Utility.ConvertToTexture(treeLineBitmap);
                return _treeLine;
            }
        }
        public static Texture2D treeMiddle {
            get {
                if(!_treeMiddle)
                    _treeMiddle = Utility.ConvertToTexture(treeMiddleBitmap);
                return _treeMiddle;
            }
        }
        public static Texture2D treeEnd {
            get {
                if(!_treeEnd)
                    _treeEnd = Utility.ConvertToTexture(treeEndBitmap);
                return _treeEnd;
            }
        }
    ...
    internal static class GUIDrawer {
        ...
        private static void DrawPrefabApply(ref Rect rect) {
            ...
            if(modifications == null || PrefabUtility.GetPrefabType(go) != PrefabType.PrefabInstance)
                return;

            var lenght = (from mod in modifications
                          where !mod.propertyPath.Contains("m_LocalRotation") &&
                          !mod.propertyPath.Contains("m_LocalPosition") &&
                          !mod.propertyPath.Contains("m_Name") &&
                          !mod.propertyPath.Contains("m_RootOrder")
                          select mod).Count();
        ...
        private static void DrawWarning(Rect rect) {
            ...
            for(int i = 0; i < components.Length; i++)
                if(!components[i]) {
                    ...
                    rect.y -= 4f;
                    rect.xMax = rect.xMin + rect.height;
                    GUI.Label(rect, Styles.warnStyle.normal.background);
                    return;
                }
        }
    ...
    internal static class Prefs {
        ...
        private static void OnPreferencesGUI() {
            ...
            EditorGUILayout.HelpBox("Inspector window must be selected for the shortcut to work", MessageType.None);
            GUI.enabled = enabled;
            ...
            warning = EditorGUILayout.Toggle("Missing behaviour warning", warning);
